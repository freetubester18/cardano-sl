...
data CommonNodeArgs = CommonNodeArgs
    ...
    , rebuildDB              :: !Bool
    , cnaBlacklistPath       :: !(Maybe FilePath)
...
commonNodeArgsParser = do
    ...
    cnaBlacklistPath <- optional $ strOption $
        long    "blacklist-file" <>
        metavar "FILEPATH" <>
        help    "Path to list of blacklisted source addresses. Funds at these \
                \addresses are not able to be spent. This will only be effective \
                \while Cardano is centrally mined/minted."
