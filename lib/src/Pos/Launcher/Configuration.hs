...
withConfigurations
    ...
    -> m r
withConfigurations mBlacklistPath cfo act = do
    logInfo ("using configurations: " <> show cfo)
    cfg <- parseYamlConfig (cfoFilePath cfo) (cfoKey cfo)
    -- Here is where we could read and add a blacklist.
    blacklist <- case mBlacklistPath of
        Nothing -> pure mempty
        Just fp -> liftIO $ readBlacklistSrcAddrs fp
...
readBlacklistSrcAddrs (BlacklistPath fp) = do
  res <- filter (not . null) . fmap Text.strip . lines <$> readFile fp
